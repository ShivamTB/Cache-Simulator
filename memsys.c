...
uns64 memsys_access_modeDE(Memsys *sys, Addr v_lineaddr, Access_Type type, uns core_id){
    uns64 delay=0;
    uns64 vpn = v_lineaddr / PAGE_SIZE;
    uns64 offset = v_lineaddr % PAGE_SIZE;
    uns64 pfn = memsys_convert_vpn_to_pfn(sys, vpn, core_id);
    Addr lineaddr= (pfn * PAGE_SIZE) + offset;
    Flag outcome;
    ...
    if(type == ACCESS_TYPE_IFETCH){
        mark_dirty = FALSE;
        //sys->stat_ifetch_access++;
            outcome=cache_access(sys->icache_coreid[core_id], lineaddr, mark_dirty, core_id);
            if(outcome==MISS){
                delay = delay + memsys_L2_access(sys, lineaddr, FALSE, core_id);
                cache_install(sys->icache_coreid[core_id], lineaddr, mark_dirty, core_id);
            }
            else{
    ...
    if(needs_dcache_access){
            outcome=cache_access(sys->dcache_coreid[core_id], lineaddr, mark_dirty, core_id);
            if(outcome==HIT){
                ...
                delay +=DCACHE_HIT_LATENCY;
                delay = delay + memsys_L2_access(sys, lineaddr, FALSE, core_id);
                cache_install(sys->dcache_coreid[core_id], lineaddr, mark_dirty, core_id);
                if(sys->dcache_coreid[core_id]->last_evicted_line.valid){
                    if (sys->dcache_coreid[core_id]->last_evicted_line.dirty){
                        Addr line = sys->dcache_coreid[core_id]->last_evicted_line.tag * sys->dcache_coreid[core_id]->num_sets;
                        line += lineaddr % sys->dcache_coreid[core_id]->num_sets;
                        memsys_L2_access(sys, line, TRUE, core_id);
                    }
                }
            }
        }
    ...
    if(outcome==HIT){
        ...
        if(sys->l2cache->last_evicted_line.valid){
            if (sys->l2cache->last_evicted_line.dirty){
                Addr line = sys->l2cache->last_evicted_line.tag * sys->l2cache->num_sets;
                line += lineaddr % sys->l2cache->num_sets;
                dram_access(sys->dram, line, TRUE);
            }
        }
    }
    return delay;
